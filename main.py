...
async def get_admin_global_stats(request: Request, email: str = Query(...)):
    try:
        ...
        for o in orders_list:
            real_orders.append({
                "email": o.get("email"),
                "name": o.get("name") or "General Purchase",
    ...
    try:
        ...
        for t in recent_trends:
            if t['keyword'] not in seen and len(final_trends) < 3:
                final_trends.append({"name": t['keyword'], "status": t['sentiment'].split()[0], **styles.get(t['sentiment'], styles["Neutral"])})
                seen.add(t['keyword'])
        return final_trends if final_trends else [{"name": "System_Active", "status": "Neutral", **styles["Neutral"]}]
