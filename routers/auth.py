...
class AuthData(BaseModel):
    ...
    email: EmailStr
    password: str
    role: str = "user"  # Captured from frontend toggle to differentiate Admin/User
...
async def register(request: Request, user: AuthData, background_tasks: BackgroundTasks):
    ...
    return {
        ...
        "user": {"name": user.name, "email": normalized_email, "role": user.role}
    }
...
async def sync_cart(request: Request, data: dict):
    ...
    if not email: raise HTTPException(status_code=400, detail="Email required for cart sync")
